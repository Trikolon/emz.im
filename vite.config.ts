...
const getFullSizeAssetPathMap = (
  ...
  for (const entry of Object.values(bundle)) {
    ...
    const id = path.basename(fullSizeSource, `.${extension}`);
  ...
  return {
    ...
    async generateBundle(_options, bundle) {
      ...
      for (const photo of photoEntries) {
        ...
        const ogImagePath = fullSizeWebpAssets.get(photo.id);
        if (!ogImagePath) {
          throw new Error(`Missing hashed WebP asset for photo ${photo.id}`);
        }
